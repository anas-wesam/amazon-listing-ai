...
# ─── Photo Studio ─────────────────────────────────────────────────────────────
@app.route("/studio/remove-bg", methods=["POST"])
def remove_bg():
    api_key = os.getenv("REMOVEBG_API_KEY", "")
    if not api_key:
        return jsonify({"error": "REMOVEBG_API_KEY not configured"}), 500
    ...
    resp = requests.post(
        ...
        data={"image_file_b64": image_b64, "size": "auto", "bg_color": "ffffff"},
        headers={"X-Api-Key": api_key},
        timeout=30
    )
    if resp.status_code != 200:
        return jsonify({"error": resp.json().get("errors", [{}])[0].get("title", "Remove.bg error")}), 400

    result_b64 = base64.b64encode(resp.content).decode("utf-8")
    return jsonify({"image_base64": f"data:image/png;base64,{result_b64}"})
